...
function Index() {
    ...
    const fetchData = async () => {
        try {
            const response_post = await apiInstance.get('posts/');
            const response_cat = await apiInstance.get('category/');
            setPosts(response_post.data);
            setCategory(response_cat.data);
        } catch (error) {
            console.log(error);
        }
    }
                                        ...
                                        <ul className="mt-3 list-style-none" style={{ listStyle: "none" }}>
                                            <li>
                                                <a href="#" className="text-dark text-decoration-none">
                                                    <i className="fas fa-user"></i> {post?.author?.full_name}
                                                </a>
                                            </li>
                                            <li className="mt-2">
                    ...
                    <nav className="d-flex mt-2">
                        ...
                        <ul className="pagination">
                            {pageNumbers.map((page) => (
                                <li key={1} className={`page-item ${currentPage === page ? 'active' : ''}`}>
                                    <button className="page-link text-dark fw-bold rounded" onClick={() => setCurrentPage(page)}>{page}</button>
                                </li>
                            ))}
                        </ul>
                        <ul className="pagination">
